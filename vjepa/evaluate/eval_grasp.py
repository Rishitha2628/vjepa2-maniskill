...
from vjepa.plan.cem_plan import cem
from vjepa.data.collect_grasp_data import scripted_action
...
def main():
    ...
    ap.add_argument("--ckpt", default="checkpoints/predictor_grasp.pt")
    a = ap.parse_args()
    ...
    np.savez("results/eval_grasp.npz", **{f"{m}_{k}": v for m, (gg, ll, ss) in rows.items()
