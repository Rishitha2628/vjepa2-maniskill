...
class KPDecoder:
    ...
    def __init__(self, path="checkpoints/keypoint.pt", device="cuda", bias_file="results/decoder_bias.npz",
...
class Decoder:
    ...
    def __init__(self, path="checkpoints/decoders.npz", device="cuda"):
        d = np.load(path)
...
def main():
    ...
    ap.add_argument("--kp-ckpt", default="checkpoints/keypoint.pt",
    ...
    np.savez("results/state_planner.npz", grasp=np.array([r[0] for r in rows], float),
